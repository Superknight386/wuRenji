...
def gen_bone(set):
    print(set)
    data = open_memmap('/root/wuRenji-main/data/{}_joint.npy'.format(set),mode='r')
    N, C, T, V, M = data.shape
    fp_sp = open_memmap('/root/wuRenji-main/data/{}_bone.npy'.format(set),dtype='float32',mode='w+',shape=(N, 3, T, V, M))
...
def merge_joint_bone_data(set):
    print(set)
    data_jpt = open_memmap('/root/wuRenji-main/data/{}_joint.npy'.format(set), mode='r')
    data_bone = open_memmap('/root/wuRenji-main/data/{}_bone.npy'.format(set), mode='r')
    N, C, T, V, M = data_jpt.shape
    data_jpt_bone = open_memmap('/root/wuRenji-main/data/{}_joint_bone.npy'.format(set), dtype='float32', mode='w+', shape=(N, 6, T, V, M))
...
def gen_motion(set,part):
    print(set, part)
    data = open_memmap('/root/wuRenji-main/data/{}_{}.npy'.format(set, part),mode='r')
    N, C, T, V, M = data.shape
    fp_sp = open_memmap('/root/wuRenji-main/data/{}_{}_motion.npy'.format(set, part),dtype='float32',mode='w+',shape=(N, 3, T, V, M))
